...
function getInitialTerrainMap(hexes: Hex[], options: GenerationOptions): Map<string, string> {
    ...
    hexes.forEach(h => { minQ = Math.min(minQ, h.q); maxQ = Math.max(maxQ, h.q); minR = Math.min(minR, h.r); maxR = Math.max(maxR, h.r); });
    ...
    const hexElevations: { hex: Hex, elevation: number }[] = hexes.map(hex => {
        ...
        if (options.highlandFormation !== 'random') {
            ...
            let nx = (hex.q - centerX) / (maxDimension / 2), ny = (hex.r - centerY) / (maxDimension / 2);
            ...
            else if (options.highlandFormation === 'circle') e_mod = 1 - Math.min(1, Math.sqrt(rx * rx + ry * ry) / Math.sqrt(2));
            // (Triangle logic omitted for brevity, but it calculates if the point is inside a rotated triangle)
        }
        if (options.highlandFormationInverse && (options.highlandFormation === 'circle' || options.highlandFormation === 'triangle')) e_mod = -e_mod;
    ...
    for (const [terrain, bias] of terrainsToPlace) {
        ...
        for (let i = currentIndex; i < endIndex; i++) {
            initialTerrainMap.set(`${hexElevations[i].hex.q},${hexElevations[i].hex.r}`, terrain);
        }
    ...
    if (currentIndex < totalHexes && terrainsToPlace.length > 0) {
        ...
        for (let i = currentIndex; i < totalHexes; i++) {
            initialTerrainMap.set(`${hexElevations[i].hex.q},${hexElevations[i].hex.r}`, lastTerrain);
        }
    }
...
function generateTerrain(hexes: Hex[], options: GenerationOptions): void {
  ...
  for (let i = 0; i < RELAXATION_PASSES; i++) {
    ...
    for (const hex of nextHexes) {
      ...
      for (const candidateTerrain of terrainTypes) {
        let score = 0;
        const neighbors = getNeighbors(hex).map(coords => currentHexesMap.get(`${coords.q},${coords.r}`)).filter(Boolean);
        ...
        if (score > maxScore) { maxScore = score; bestTerrain = candidateTerrain; }
      }
  ...
  hexes.forEach(h => { h.terrain = finalHexesMap.get(`${h.q},${h.r}`)?.terrain || h.terrain; });
}
...
function placeHoldings(hexes: Hex[], sizeForDensity: number, numHoldings: number): { q: number; r: number } {
    ...
    while (placedHoldings.length < numHoldings && validHexes.length > 0) {
        ...
        if (!hex.holding && !placedHoldings.some(h => getAxialDistance(h, hex) < sizeForDensity / 4)) {
            hex.holding = getRandomElement(HOLDING_TYPES);
            placedHoldings.push(hex);
        }
    }
    if (placedHoldings.length > 0) return { q: placedHoldings[0].q, r: placedHoldings[0].r };
    const fallbackHex = hexes.find(h => h.q === 0 && h.r === 0) || hexes[0];
    if (fallbackHex) { fallbackHex.holding = 'castle'; return { q: fallbackHex.q, r: fallbackHex.r }; }
    return { q: 0, r: 0 };
}
...
function placeLandmarks(hexes: Hex[], landmarkOptions: { [key: string]: number }): void {
    ...
    Object.entries(landmarkOptions).forEach(([type, count]) => {
        for (let i = 0; i < count && validHexes.length > 0; i++) {
            const index = Math.floor(Math.random() * validHexes.length);
            validHexes.splice(index, 1)[0].landmark = type;
        }
    });
}
...
export function generateRealm(options: GenerateRealmOptions, genOptions: GenerationOptions): Realm {
  let hexes: Hex[], sizeForDensity: number;
  ...
  if (genOptions.numMyths > 0 && myths.length < genOptions.numMyths) {
    throw new Error(`Could not place all myths. Only placed ${myths.length} of ${genOptions.numMyths}. Try reducing 'Myth Min Distance' or the number of myths.`);
  }
