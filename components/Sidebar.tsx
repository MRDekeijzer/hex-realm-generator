...
const renderSelect = (label: string, value: string, options: {id: string, label: string}[], onChange: (e: React.ChangeEvent<HTMLSelectElement>) => void) => (
    <div className="mb-4">
        ...
        <select value={value} onChange={onChange} className="w-full p-2 bg-[#324446] border border-[#41403f] rounded-md focus:outline-none focus:ring-2 focus:ring-[#736b23]">
  ...
  return (
    <aside className="w-80 bg-[#191f29] border-l border-[#41403f] p-4 flex flex-col">
      ...
      <div className="flex-grow overflow-y-auto">
        {renderSelect('Terrain', selectedHex.terrain, tileSets.terrain, (e) => handleChange('terrain', e.target.value))}
        
        <div className="mb-4">
            <label className="block text-sm font-medium text-[#a7a984] mb-1">Holding</label>
            <select value={selectedHex.holding || ''} onChange={(e) => handleChange('holding', e.target.value)} className="w-full p-2 bg-[#324446] border border-[#41403f] rounded-md focus:outline-none focus:ring-2 focus:ring-[#736b23]">
                <option value="">None</option>
                {tileSets.holding.map(opt => <option key={opt.id} value={opt.id}>{opt.label}</option>)}
            </select>
        </div>
        ...
        <div className="mb-4">
            <label className="block text-sm font-medium text-[#a7a984] mb-1">Landmark</label>
            <select value={selectedHex.landmark || ''} onChange={(e) => handleChange('landmark', e.target.value)} className="w-full p-2 bg-[#324446] border border-[#41403f] rounded-md focus:outline-none focus:ring-2 focus:ring-[#736b23]">
                <option value="">None</option>
                {tileSets.landmark.map(opt => <option key={opt.id} value={opt.id}>{opt.label}</option>)}
            </select>
        </div>
