...
const rgbaToHexOpacity = (rgba: string): { hex: string; opacity: number } => {
    ...
    return {
        hex: `#${toHex(parseInt(match[1]))}${toHex(parseInt(match[2]))}${toHex(parseInt(match[3]))}`,
 ...
 */
export const GridSettingsPopover = React.forwardRef<HTMLDivElement, GridSettingsPopoverProps>(({ viewOptions, setViewOptions }, ref) => {
    ...
    const handleGridColorChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (rgb) {
            setViewOptions(v => ({ ...v, gridColor: `rgba(${rgb.r}, ${rgb.g}, ${rgb.b}, ${gridOpacity})` }));
        }
    };
    ...
    const handleGridOpacityChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (rgb) {
            setViewOptions(v => ({ ...v, gridColor: `rgba(${rgb.r}, ${rgb.g}, ${rgb.b}, ${parseFloat(e.target.value)})` }));
        }
    };
    
    const handleResetGridSettings = () => {
        setViewOptions(v => ({ ...v, showGrid: true, gridColor: DEFAULT_GRID_COLOR, gridWidth: DEFAULT_GRID_WIDTH }));
    };

    return (
        <div ref={ref} className="absolute top-full mt-2 left-0 bg-[#18272e] border border-[#41403f] rounded-lg shadow-xl p-4 z-20 w-64">
            <div className="space-y-4">
                ...
                <div>
                    ...
                    <div className="flex items-center gap-2">
                         <button
                            id="grid-color-btn"
                            onClick={() => isCustomGridColor ? setViewOptions(v => ({ ...v, gridColor: DEFAULT_GRID_COLOR })) : gridColorInputRef.current?.click()}
                        ...
                        >
                            <input ref={gridColorInputRef} type="color" value={gridHexColor} onChange={handleGridColorChange} className="opacity-0 w-0 h-0 absolute pointer-events-none" />
                ...
                <div>
                    ...
                    <div className="flex items-center gap-2">
                        <input id="grid-opacity" type="range" min="0" max="1" step="0.01" value={gridOpacity} onChange={handleGridOpacityChange} className="w-full h-2 bg-[#324446] rounded-lg appearance-none cursor-pointer" aria-label="Grid opacity" />
                 ...
                 <div>
                    ...
                    <input id="grid-width" type="number" value={viewOptions.gridWidth} onChange={(e) => setViewOptions(v => ({...v, gridWidth: Math.max(0.1, parseFloat(e.target.value)) || 1}))} min="0.1" step="0.1" className="w-full bg-[#324446] p-2 text-sm font-medium text-[#a7a984] focus:outline-none focus:ring-2 focus:ring-[#736b23] rounded-md" aria-label="Grid border width" />
                </div>
            </div>
            <div className="mt-4 pt-4 border-t border-[#41403f]">
                <button onClick={handleResetGridSettings} className="w-full flex items-center justify-center gap-2 px-3 py-2 text-sm font-medium text-[#a7a984] bg-[#324446] rounded-md hover:bg-[#435360] transition-colors">
