              ...
              {realm.myths.sort((a, b) => a.id - b.id).map(myth => {
                ...
                return (
                  <li key={myth.id} className="bg-[#18272e] rounded-md overflow-hidden transition-all duration-300">
                    <button onClick={() => handleMythClick(myth)} className="w-full text-left p-3 hover:bg-[#435360] transition-colors flex justify-between items-center disabled:cursor-not-allowed" aria-expanded={isExpanded || isRelocating} disabled={!!relocatingMythId}>
                      <div>
                    ...
                    {(isExpanded || isRelocating) && (
                      <div className="p-3 border-t border-[#41403f]/50 bg-[#18272e]/50 space-y-3">
                        <div>
                            ...
                            <input id={`myth-name-${myth.id}`} type="text" value={myth.name} onChange={(e) => onUpdateMyth({ ...myth, name: e.target.value })} className="w-full bg-[#324446] p-2 text-sm font-medium text-[#a7a984] focus:outline-none focus:ring-2 focus:ring-[#736b23] rounded-md" />
                        </div>
                        <div className="flex gap-2">
                            <button onClick={() => onToggleRelocateMyth(myth.id)} className={`w-full flex items-center justify-center gap-2 px-3 py-2 text-sm font-medium rounded-md transition-colors ${isRelocating ? 'bg-[#435360]/80 text-[#eaebec] hover:bg-[#435360]' : 'bg-[#324446] text-[#a7a984] hover:bg-[#435360]'}`}>
                                <Icon name="move" className="w-4 h-4" />
                                {isRelocating ? 'Cancel Relocate' : 'Relocate'}
                            </button>
                            <button onClick={() => handleRemoveClick(myth)} className="w-full flex items-center justify-center gap-2 px-3 py-2 text-sm font-medium text-[#a7a984] bg-[#60131b]/50 rounded-md hover:bg-[#60131b]/80 border border-[#60131b]/80 transition-colors">
