...
import { TILE_SETS, OVERLAY_ICONS, SPECIAL_POI_ICONS } from '../constants';
...
const PoiButton = ({ item, type, isSelected, onClick, svgText }: { item: Tile; type: string; isSelected: boolean; onClick: () => void; svgText?: string; }) => {
    ...
    return (
        ...
        >
            <div 
              className="w-12 h-12 rounded-lg flex items-center justify-center bg-gray-700"
            >
              {svgText ? (
                  <svg 
                      width="32" 
                      height="32" 
                      viewBox="0 0 24 24" 
                      className={type === 'overlay' || type === 'action' ? 'text-white' : 'text-gray-900'}
                      fill={'none'}
                      stroke={'currentColor'}
  ...
  return (
    <aside className="w-80 bg-gray-900 border-l border-gray-700 p-4 flex flex-col">
      ...
      <div className="flex-grow overflow-y-auto pr-2 space-y-6">
        <p className="text-sm text-gray-400">Select an item, then click a hex to place it.</p>
